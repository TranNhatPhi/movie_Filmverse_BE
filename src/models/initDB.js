...
const Episode = require("./episode"); // Import Episode model
...
const initDatabase = async () => {
    try {
        ...
        // 1. Kiểm tra và tạo phim mẫu nếu chưa tồn tại
        let sampleMovie = await Movie.findOne({ slug: "sample-movie" });
        if (!sampleMovie) {
            sampleMovie = await Movie.create({
                name: "Sample Movie",
                slug: "sample-movie",
                // ... các trường khác của Movie (nếu có)
            });
            console.log("✅ Sample Movie has been created.");
        } else {
            console.log("ℹ️ Sample Movie already exists. Skipping creation.");
        }
